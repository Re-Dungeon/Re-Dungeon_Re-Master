...
import { getRmNotas } from 'service/storage';
...
const Dashboard = () => {
  ...
  const { cenas, nodes, edges, loading: loadingGrafo } = useCampanhaGrafo(campanhaAtiva);
  ...
  const carregarNotas = useCallback(() => {
    ...
    return getRmNotas()
      .then(todas => setNotas(todas.filter(n => n.campanhaId === campanhaAtiva.id)))
      .finally(() => setLoadingNotas(false));
  }, [campanhaAtiva]);

  useEffect(() => {
    Promise.resolve().then(() => carregarNotas());
  }, [carregarNotas]);
  ...
  const loading = loadingGrafo || loadingNotas;
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Paper elevation={0} sx={cardSx}>
              <Typography
                variant="caption"
                sx={{ color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}
              >
                Cena Atual
              </Typography>
              {cenaAtual ? (
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                    ...
                    {estadoCenaAtual && (
                      <Chip
                        ...
                        size="small"
                        sx={{ background: estadoCenaAtual.cor, color: '#fff', fontWeight: 600 }}
                      />
                    )}
                  </Box>
                  {cenaAtual.objetivo && (
                ...
                <Box sx={{ mt: 1.5 }}>
                  <Typography variant="body2" sx={{ color: 'var(--text-muted)', mb: 1.5 }}>
                    Nenhuma cena marcada como atual ainda. Abra uma cena no fluxograma e marque-a.
                  </Typography>
                  <Button
                    size="small"
                    variant="outlined"
                  ...
                  proximasCenas.map(cena => (
                    <Button
                      ...
                      sx={{
                        ...
                        '&:hover': { background: 'var(--bg-secondary)', opacity: 0.8 },
                      }}
                    >
                      {cena.titulo}
                    </Button>
              ...
              <Box sx={{ mt: 1.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                ...
                {notas.length > 0 && (
                  <Button
                    ...
                    sx={{ alignSelf: 'flex-start', color: 'var(--color-accent)', mt: 0.5 }}
                  >
                    Ver todas →
                  </Button>
                )}
