...
const MISSOES_MOCK = [
  ...
  { id: 'missao2', campanhaId: 'c1', titulo: 'Investigar a Praça', status: 'nao_iniciada' },
];
...
describe('Missoes (lista da campanha ativa)', () => {
  ...
  it('lista as missões da campanha ativa com status e progresso', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Resgatar o Prefeito')).toBeInTheDocument());
  ...
  it('remove uma missão', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Resgatar o Prefeito')).toBeInTheDocument());
    await user.click(screen.getByLabelText('Remover missão Resgatar o Prefeito'));
